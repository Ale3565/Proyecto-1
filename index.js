...
const highScoreKey = '__Puntuacion Alta';
const getHighScore = () => {
...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `PUNTUACIÓN: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `CUBOS DESTRUIDOS: ${state.game.cubeCount}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = 'Nueva Puntuación Alta!';
			} else {
				highScoreLblNode.textContent = `Puntuación Alta: ${formatNumber(getHighScore())}`;
			}
